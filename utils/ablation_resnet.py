...
"""
from tensorflow.keras.layers import MaxPool2D, GlobalAvgPool2D,Add, ReLU, Dense,Input, Conv2D, BatchNormalization,GlobalAveragePooling2D
...
def custom_resnet_ablation(res_model=[],ablation='',custom_input=(224,224,3),n_classes=1000,model_name='custom_resnet',debug=False):
    ratio=16
    if not ablation in ['standard','pre','post','identity']:
        print('ablation can only be in ',['standard','pre','post','identity'])
        print('returning non SE version')
    ...
    def first_resnet_block(input_x, filters,kernels, big_stride,name): 
        ...
        if ablation=='pre':
            ...
            #squeeze layer
            F_sq = GlobalAveragePooling2D(name=name+'_SE_squeeze')(input_x) #squeeze
            #Excitation with 2 fully connected layers
            F_ex = Dense(out_dim / ratio,activation='relu', name=name+'_SE_fc1')(F_sq)
            F_ex = Dense(out_dim,activation='sigmoid', name=name+'_SE_fc2')(F_ex)
            #Output : rescaling
            F_ex = tf.reshape(F_ex, [-1,1,1,out_dim])
            x=tf.keras.layers.multiply([input_x,F_ex], name=name+'_SE_scaling')
        
        
        x = conv_and_activation_block(x, filters=filters[0], kernel_size=kernels[0], strides=big_stride,name=name+'_1')  
        ...
        if ablation=='identity':
            ### adding SE layers
            out_dim=shortcut.shape[-1]
            #squeeze layer
            F_sq = GlobalAveragePooling2D(name=name+'_SE_squeeze')(shortcut) #squeeze
            #Excitation with 2 fully connected layers
            F_ex = Dense(out_dim / ratio,activation='relu', name=name+'_SE_fc1')(F_sq)
            F_ex = Dense(out_dim,activation='sigmoid', name=name+'_SE_fc2')(F_ex)
            #Output : rescaling
            F_ex = tf.reshape(F_ex, [-1,1,1,out_dim])
            shortcut=tf.keras.layers.multiply([shortcut,F_ex], name=name+'_SE_scaling')
        ...
        if ablation=='standard':
            ...
            out_dim=x.shape[-1]
            #squeeze layer
            F_sq = GlobalAveragePooling2D(name=name+'_SE_squeeze')(x) #squeeze
            #Excitation with 2 fully connected layers
            F_ex = Dense(out_dim / ratio,activation='relu', name=name+'_SE_fc1')(F_sq)
            F_ex = Dense(out_dim,activation='sigmoid', name=name+'_SE_fc2')(F_ex)
            #Output : rescaling
            F_ex = tf.reshape(F_ex, [-1,1,1,out_dim])
            x=tf.keras.layers.multiply([x,F_ex], name=name+'_SE_scaling')
            
        #add the two sequences
        x = Add(name=name+'_add')([shortcut,x])    #skip connection     
        x = ReLU(name=name+'_relu')(x)   
        # post comes after the relu
        if ablation=='post':
            ### adding SE layers
            out_dim=x.shape[-1]
            #squeeze layer
    ...
    def block_with_skip(input_x, filters,kernels,name):
        ...
        x = conv_and_activation_block(x, filters=filters[0], kernel_size=kernels[0], strides=1,name=name+'_1')
        #other conv layers have stride 1
        for i in range(len(filters[1:-1])): 
            x = conv_and_activation_block(x, filters=filters[1+i], kernel_size=kernels[i+1], strides=1,name=name+'_'+str(i+2))     
        ...
        if ablation=='identity':
            ...
            #Output : rescaling
            F_ex = tf.reshape(F_ex, [-1,1,1,out_dim])
            input_x=tf.keras.layers.multiply([input_x,F_ex], name=name+'_SE_scaling')
        #skip node
        x = Add(name=name+'_add')([input_x,x])    
        x = ReLU(name=name+'_relu')(x)
        
        # post if after relu as indicatied by the paper
...
def custom_resnet18_ablation(ablation='standard',input_shape=(224,224,3),n_classes=1000,name='custom_resnet18',debug=False):
    return  custom_resnet([[[3,3],[64,64],2],[[3,3],[128,128],2],[[3,3],[256,256],2],[[3,3],[512,512],2]],ablation,input_shape,n_classes,name,debug)

# builds resnet34
# https://pytorch.org/assets/images/resnet.png for more information
def custom_resnet34_ablation(ablation='standard',input_shape=(224,224,3),n_classes=1000,name='custom_resnet34',debug=False):
    return  custom_resnet([[[3,3],[64,64],3],[[3,3],[128,128],4],[[3,3],[256,256],6],[[3,3],[512,512],3]],ablation,input_shape,n_classes,name,debug)

def custom_resnet50_ablation(ablation='standard',input_shape=(224,224,3),n_classes=1000,name='custom_resnet50',debug=False):
    return  custom_resnet([],ablation,input_shape,n_classes,name,False)

# builds resnet101
# https://pytorch.org/assets/images/resnet.png for more information
def custom_resnet101_ablation(ablation='standard',input_shape=(224,224,3),n_classes=1000,name='custom_resnet101',debug=False):
    return  custom_resnet([[1,3,1],[64,64,256],3],[[1,3,1],[128,128,512],4],[[1,3,1],[256,256,1024],23],[[1,3,1],[512,512,2048],3],ablation,input_shape,n_classes,name,debug)

# builds resnet152
# https://pytorch.org/assets/images/resnet.png for more information
def custom_resnet152_ablation(ablation='standard',input_shape=(224,224,3),n_classes=1000,name='custom_resnet152',debug=False):
    return  custom_resnet([[1,3,1],[64,64,256],3],[[1,3,1],[128,128,512],4],[[1,3,1],[256,256,1024],36],[[1,3,1],[512,512,2048],3],ablation,input_shape,n_classes,name,debug)
